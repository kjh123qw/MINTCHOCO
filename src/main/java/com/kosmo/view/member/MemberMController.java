...
public class MemberMController {
	...
	@RequestMapping(value="/sortAssessList.do", method=RequestMethod.POST)
	public String sortFavList(@RequestParam(value="sort") String sort, AssessmentMVO vo, AssessmentMDAO assessDAO, MemberVO member, MemberMDAO memberDAO, Model model, HttpSession session) {
		session.setAttribute("memberNum","2");
		String memberNum = (String) session.getAttribute("memberNum");
		model.addAttribute("detail_info", memberDAO.getDetailInfo(memberNum));
		model.addAttribute("assess_cnt", memberDAO.getAssessmentCnt(memberNum));
		if (sort.equals("sub")) {
			...
			return "/assessment.do";
		}
		return "member/myp_assess_list.jsp";
	}
	
	@RequestMapping(value="/sortFavList.do", method=RequestMethod.POST)
	public String sortFavList(@RequestParam(value="sort") String sort,FavoriteVO vo, FavoriteDAO favoriteDAO, MemberVO member, MemberMDAO memberDAO, Model model, HttpSession session) {
		session.setAttribute("memberNum","2");
		String memberNum = (String) session.getAttribute("memberNum");
		model.addAttribute("detail_info", memberDAO.getDetailInfo(memberNum));
		model.addAttribute("assess_cnt", memberDAO.getAssessmentCnt(memberNum));
