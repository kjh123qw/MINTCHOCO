...
public class AssessmentController {
	...
	public void assDelAfter(AssessmentVO VO, AssessmentDAO DAO)
	{
		...
		if(assRankList.get(2).getLikes() < VO.getLikes() || (assRankList.get(2).getLikes() == VO.getLikes() && compare < 0)) 
		{
			...
			for(int i = 0; i < tagList.size(); i++)
			{
				...
				{
					DAO.deleteTagMapping(tagList.get(i), VO.getMovieNumber());
				...
				if(0 == i)
				{
					...
					List<String> list = getTagList(assRankList.get(2).getAssessContent());
					for(String str : list)
					{
						int count = DAO.insertTagMapping(str, VO.getMovieNumber());
						if(0 < count)
	...
	//추천 눌렀을시
	public void assPushLikesAfter(AssessmentVO VO, AssessmentDAO DAO)
	{
		List<String> tagList = getTagList(VO.getAssessContent());
		...
		for(int i = 0; i < assRankList.size() - 1; i++)
		{
			...
			if(VO.getAssessId().equals(assRankList.get(i).getAssessId()))
			{
				...
				List<String> rank4TagList = getTagList(assRankList.get(3).getAssessContent());
